...
export default function GroupSearch({ onSearchResults, onClear }) {
  ...
  const search = useCallback(async (searchQuery) => {
    ...
    try {
      const response = await fetch(
        `/api/search?q=${encodeURIComponent(searchQuery)}&type=groups`
      );
      const data = await response.json();
      if (response.ok) {
        onSearchResults?.(data.groups || []);
      }
    } catch (error) {
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        placeholder="Search groups by name, location, or description..."
