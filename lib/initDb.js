...
async function modifyColumnType(connection, table, column, newDefinition) {
  // This function modifies an existing column's type
  const quotedTable = table.includes('`') ? table : `\`${table}\``;
  try {
    await connection.query(
      `ALTER TABLE ${quotedTable} MODIFY COLUMN ${newDefinition}`
    );
  } catch (error) {
    // Ignore errors if column doesn't exist or is already the right type
    console.log(`Note: Could not modify column ${column}: ${error.message}`);
  }
}
...
export async function initializeDatabase() {
  ...
  try {
    ...
    try {
      ...
      // Migrate avatarUrl from VARCHAR to LONGTEXT if it exists as VARCHAR
      ...
      // Check if we need to seed a past event
      const [eventRows] = await connection.query(
        'SELECT COUNT(*) as count FROM events'
      );

      if (eventRows[0].count === 0) {
        console.log('📅 Seeding sample past event...');

        // Create a past event (2 weeks ago)
        const pastEventTime = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);
        const pastEventCreatedAt = new Date(
          Date.now() - 21 * 24 * 60 * 60 * 1000
        );

        const [eventResult] = await connection.query(
          `INSERT INTO events (creatorId, title, location, description, eventTime, createdAt, latitude, longitude) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            1,
            'Community Beach Cleanup',
            'Ocean Beach, San Francisco',
            'Join us for a morning of cleaning up our beautiful coastline! We provided gloves, bags, and refreshments. Thanks to everyone who came out - we collected over 200 lbs of trash and recyclables!',
            pastEventTime,
            pastEventCreatedAt,
            37.7594, // Ocean Beach latitude
            -122.5107, // Ocean Beach longitude
          ]
        );
        ...
        await connection.query(
          ...
          [eventResult.insertId, 1, pastEventCreatedAt]
        );

        console.log('✅ Seeded 1 past event');
      }
